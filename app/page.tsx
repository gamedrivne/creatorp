...
function Preview({ image, sound, quotes, onClose, isRecording = false, selectedFont, selectedColor, selectedAnimation, fontSize, duration }: PreviewProps) {
  ...
  const [actualDuration, setActualDuration] = useState(0); // NOUVEAU : Suivre la durée réelle
  const containerRef = useRef<HTMLDivElement>(null);
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const animationFrameRef = useRef<number | null>(null);
  const startTimeRef = useRef<number>(0); // NOUVEAU : Pour calculer la durée réelle

  useEffect(() => {
    setIsMounted(true);
  }, []);

  // FIX 1 : Utiliser les mêmes paramètres que la prévisualisation pour les particules vidéo
  const createVideoParticle = (index: number, animType: string, frame: number): Particle => {
    ...
    // Utiliser les mêmes vitesses et tailles que la version prévisualisation
    switch(animType) {
      case 'snow':
  ...
  const startPreview = async () => {
    try {
      ...
      await newAudio.play();
      setIsStarted(true);
      setActualDuration(0); // Réinitialiser le compteur
  ...
  // FIX 2 : Utiliser la prop duration au lieu de la valeur hardcodée
  const startVideoGeneration = async (audioElement: HTMLAudioElement) => {
    try {
      setRecordingStatus(`🎬 Initialisation ${duration}s...`);
      startTimeRef.current = Date.now(); // NOUVEAU : Commencer le comptage
      
      const canvas = canvasRef.current;
      if (!canvas) return;

      // FIX CRITIQUE : Canvas doit être visible pour la capture
      ...
      const fps = 30;
      const totalFrames = fps * duration; // FIX : Utiliser la durée réelle
      ...
      // FIX 3 : Boucle de rendu stable avec timing exact
      const renderFrame = () => {
        ...
        // Mettre à jour et dessiner les particules avec les mêmes effets que la prévisualisation
        if (selectedAnimation !== 'none' && particlesArray.length > 0) {
          particlesArray.forEach((particle, index) => {
            ...
            // Effets spéciaux identiques à la prévisualisation
            ...
            // Dessiner la particule avec les mêmes effets visuels que la prévisualisation
            ctx.save();
            ctx.globalAlpha = particle.opacity;
            
            // Ajouter glow/shadow comme dans la version CSS
        ...
        // FIX 4 : Calcul précis du temps écoulé
