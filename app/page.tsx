...
function Preview({ image, sound, quotes, onClose, isRecording = false, selectedFont, selectedColor, selectedAnimation, fontSize, duration }: PreviewProps) {
  ...
  const startVideoGeneration = async (audioElement: HTMLAudioElement) => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        setRecordingStatus('✅ Vidéo téléchargée !');
  ...
  useEffect(() => {
    ...
    const createParticle = (type: string): Particle => {
      ...
      return {
        id, x: Math.random() * 450, y: -20, vx: 0, vy: Math.random() * 2 + 1,
        size: Math.random() * 8 + 4, opacity: 0.8, color: '#FFFFFF'
      };
    };
    ...
    const interval = setInterval(() => {
      setParticles(prev => {
        ...
        const updated = prev.map(p => ({ ...p, x: p.x + p.vx, y: p.y + p.vy })).filter(p => p.y < 850);
    ...
    <div className="fixed inset-0 z-50 bg-gradient-to-br from-purple-900 via-blue-900 to-pink-900 flex items-center justify-center p-3">
      ...
      <div ref={containerRef} className="relative bg-black rounded-2xl sm:rounded-3xl overflow-hidden shadow-2xl border-2 sm:border-4 border-purple-500" style={{ width: '100%', maxWidth: '450px', aspectRatio: '9/16' }}>
        ...
        {isStarted && !isRecording && particles.map(particle => (
          <div key={particle.id} className="absolute rounded-full pointer-events-none" style={{ left: `${particle.x}px`, top: `${particle.y}px`, width: `${particle.size}px`, height: `${particle.size}px`, opacity: particle.opacity, backgroundColor: particle.color, boxShadow: `0 0 ${particle.size}px ${particle.color}` }} />
        ))}
        ...
        <div className="lg:w-2/3">
          ...
          <div className="space-y-4 sm:space-y-5">
            <div className="bg-white bg-opacity-10 backdrop-blur-md p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-white border-opacity-20">
              ...
              {selectedImage && <p className="text-xs sm:text-sm mb-2 bg-green-500 text-white px-2 py-1 rounded inline-block">✓ Image sélectionnée</p>}
              <button onClick={loadImages} disabled={loading} className="bg-gradient-to-r from-purple-500 to-pink-500 w-full py-3 sm:py-4 rounded-xl disabled:opacity-50 transition-all font-bold text-base sm:text-lg text-white hover:scale-105">
            ...
            <div className={`bg-white bg-opacity-10 backdrop-blur-md p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-white border-opacity-20 ${!selectedImage ? 'opacity-50' : ''}`}>
              ...
              {selectedSound && <p className="text-xs sm:text-sm mb-2 bg-green-500 text-white px-2 py-1 rounded inline-block">✓ {selectedSound.title || 'Musique sélectionnée'}</p>}
              <button onClick={loadSounds} disabled={!selectedImage || loading} className="bg-gradient-to-r from-blue-500 to-cyan-500 w-full py-3 sm:py-4 rounded-xl disabled:opacity-50 transition-all font-bold text-base sm:text-lg text-white hover:scale-105">
            ...
            <div className={`bg-white bg-opacity-10 backdrop-blur-md p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-white border-opacity-20 ${!selectedSound ? 'opacity-50' : ''}`}>
              ...
              {selectedCategory && <p className="text-xs sm:text-sm mb-2 bg-green-500 text-white px-2 py-1 rounded inline-block">✓ {selectedCategory} ({quotes.length} citations)</p>}
              <button onClick={() => setShowCategoryPicker(true)} disabled={!selectedSound || loading} className="bg-gradient-to-r from-orange-500 to-red-500 w-full py-3 sm:py-4 rounded-xl disabled:opacity-50 transition-all font-bold text-base sm:text-lg text-white hover:scale-105">
            ...
            {quotes.length > 0 && (
              <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-4 sm:p-6 rounded-xl sm:rounded-2xl shadow-2xl border-2 sm:border-4 border-white border-opacity-40">
                ...
                <button onClick={() => { setIsRecordingMode(false); setShowPreview(true); }} className="w-full bg-white text-green-600 py-3 sm:py-4 rounded-xl mb-2 sm:mb-3 transition-all font-bold text-base sm:text-lg hover:scale-105 shadow-lg">
                  👁️ Prévisualiser
                </button>
                <button onClick={() => { setIsRecordingMode(true); setShowPreview(true); }} className="w-full bg-red-600 hover:bg-red-700 text-white py-3 sm:py-4 rounded-xl transition-all font-bold text-base sm:text-lg hover:scale-105 shadow-lg">
                  🎥 Création vidéo ({selectedDuration}s)
                </button>
              </div>
            )}
          </div>
