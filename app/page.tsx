...
interface PreviewProps {
  ...
  fontSize: number;
  duration: number; // Nouveau paramètre
}
...
// NOUVELLE CONSTANTE : Options de durée
...
function Preview({ image, sound, quotes, onClose, isRecording = false, selectedFont, selectedColor, selectedAnimation, fontSize, duration }: PreviewProps) {
  ...
  // CRITICAL FIX : Simplifier la création de particules pour la vidéo
  const createVideoParticle = (index: number, animType: string, frame: number): Particle => {
    ...
    switch(animType) {
      case 'snow':
        return { id: index, x: baseX, y: -20, size: Math.random() * 8 + 4, vx: Math.sin(index) * 0.5, vy: Math.random() * 2 + 1, opacity: Math.random() * 0.6 + 0.4, color: colors.snow };
      case 'stars':
        return { id: index, x: baseX, y: baseY, size: Math.random() * 3 + 1, vx: 0, vy: 0, opacity: Math.random(), color: colors.stars };
      case 'rain':
        return { id: index, x: baseX, y: -20, size: Math.random() * 3 + 2, vx: 0, vy: Math.random() * 5 + 5, opacity: 0.7, color: colors.rain };
      case 'bubbles':
        return { id: index, x: baseX, y: 1280 + 20, size: Math.random() * 20 + 10, vx: Math.random() * 2 - 1, vy: -(Math.random() * 2 + 1), opacity: 0.5, color: colors.bubbles };
      case 'confetti':
        return { id: index, x: baseX, y: -20, size: Math.random() * 8 + 4, vx: Math.random() * 4 - 2, vy: Math.random() * 3 + 2, opacity: 1, color: colors.confetti[index % 5] };
      case 'leaves':
        return { id: index, x: baseX, y: -20, size: Math.random() * 12 + 8, vx: Math.sin(index * 2) * 2, vy: Math.random() * 1.5 + 0.5, opacity: 0.8, color: colors.leaves[index % 3] };
      case 'butterflies':
        return { id: index, x: baseX, y: baseY, size: Math.random() * 15 + 10, vx: Math.sin(index * 0.1) * 3, vy: Math.random() * 1 + 0.5, opacity: 0.9, color: colors.butterflies[index % 3] };
      case 'hearts':
        return { id: index, x: baseX, y: -20, size: Math.random() * 15 + 10, vx: Math.random() * 2 - 1, vy: Math.random() * 2 + 1, opacity: 0.8, color: colors.hearts };
      case 'particles':
        return { id: index, x: baseX, y: baseY, size: Math.random() * 4 + 2, vx: Math.random() * 0.5 - 0.25, vy: Math.random() * 0.5 - 0.25, opacity: Math.random() * 0.5 + 0.5, color: colors.particles };
      case 'light':
        return { id: index, x: baseX, y: baseY, size: Math.random() * 6 + 3, vx: 0, vy: 0, opacity: Math.random() * 0.3 + 0.2, color: colors.light };
      case 'fire':
        return { id: index, x: baseX, y: 1280 + 20, size: Math.random() * 20 + 10, vx: Math.random() * 2 - 1, vy: -(Math.random() * 3 + 2), opacity: Math.random() * 0.7 + 0.3, color: colors.fire[index % 3] };
      case 'smoke':
        return { id: index, x: baseX, y: 1280 + 20, size: Math.random() * 30 + 20, vx: Math.random() * 1 - 0.5, vy: -(Math.random() * 1 + 0.5), opacity: Math.random() * 0.3 + 0.1, color: colors.smoke };
      case 'clouds':
      ...
      case 'petals':
        return { id: index, x: baseX, y: -20, size: Math.random() * 10 + 6, vx: Math.sin(index * 0.01) * 1.5, vy: Math.random() * 1.5 + 0.8, opacity: 0.7, color: colors.petals[index % 3] };
      case 'sparkles':
        return { id: index, x: baseX, y: baseY, size: Math.random() * 5 + 2, vx: 0, vy: 0, opacity: Math.random(), color: colors.sparkles[index % 3] };
      case 'aurora':
        return { id: index, x: baseX, y: baseY, size: Math.random() * 100 + 50, vx: Math.random() * 0.2 - 0.1, vy: Math.random() * 0.2, opacity: 0.1, color: colors.aurora[index % 3] };
      case 'plasma':
        return { id: index, x: baseX, y: baseY, size: Math.random() * 80 + 40, vx: Math.random() * 0.5 - 0.25, vy: Math.random() * 0.5 - 0.25, opacity: 0.2, color: colors.plasma[index % 3] };
      case 'waves':
        return { id: index, x: baseX, y: 1280 - Math.abs(Math.sin(frame * 0.1) * 200), size: Math.random() * 40 + 30, vx: 0, vy: Math.sin(frame * 0.001) * 2, opacity: 0.3, color: colors.waves };
      case 'circles':
  ...
  // CRITICAL FIX : Simplifier et stabiliser la génération vidéo
  const startVideoGeneration = async (audioElement: HTMLAudioElement) => {
    try {
      ...
      // FIX : Assurer que le canvas est visible pour la capture
      ...
      setRecordingStatus(`🎬 Configuration audio/vidéo (${duration}s)...`);
      const fps = 30;
      const totalFrames = fps * duration;
      const framesPerQuote = fps * 5; // 5 secondes par citation
      ...
      if (selectedAnimation !== 'none') {
        particlesArray = Array.from({ length: 40 }, (_, i) => createVideoParticle(i, selectedAnimation, 0));
      }

      let frameCount = 0;
      const startTime = Date.now();

      // Boucle de rendu stable
      const renderFrame = () => {
  ...
  // Logique pour l'aperçu (non-enregistrement)
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900 bg-opacity-95 flex items-center justify-center">
      ...
      {/* FIX : Canvas visible pour capture mais hors écran */}
      <canvas 
        ref={canvasRef} 
        style={{ 
          position: 'absolute', 
...
function Home() {
  ...
  const [showDurationPicker, setShowDurationPicker] = useState(false); // NOUVEAU
  ...
  const [selectedDuration, setSelectedDuration] = useState(60); // NOUVEAU
  ...
  if (showPreview && selectedImage && selectedSound) {
    return <Preview 
      ...
      fontSize={fontSize}
      duration={selectedDuration} // Passer la durée
    />;
  }

  // NOUVEAU : Picker de durée
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white p-4">
        <div className="max-w-4xl mx-auto">
          ...
          <h2 className="text-2xl font-bold mb-4">Choisir la durée de la vidéo</h2>
      ...
      <div className="flex gap-6 max-w-7xl mx-auto">
        ...
        <div className="w-2/3">
          ...
          <div className="space-y-4">
            ...
            {/* NOUVEAU : Choix de durée */}
            <div className={`bg-gray-800 p-6 rounded-lg ${!selectedCategory ? 'opacity-50' : ''}`}>
              <h2 className="text-xl font-semibold mb-2">Étape 5: Durée vidéo</h2>
              <button 
                onClick={() => setShowDurationPicker(true)} 
                disabled={!selectedCategory} 
                className="w-full bg-yellow-600 hover:bg-yellow-700 py-3 rounded-lg disabled:bg-gray-600 transition"
              >
